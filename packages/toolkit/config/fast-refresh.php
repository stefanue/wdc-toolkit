...
// phpcs:ignoreFile
/**
 * This file is generated by wdc-toolkit and copied into your dist folder, do not modifies this file directly.
 */
namespace WDCToolkit;
...
add_action( 'init',
	...
	function() {
		...
		if ( ! $dist_url || ! $dist_path ) {
			wp_die( 'wdc-toolkit error: you must defined call WDCToolkit\set_dist_url_path with the URL and path to dist folfer to get fast refresh to work' );
		}
		...
		wp_register_script(
			'wdc-toolkit-react-fast-refresh-entry',
		...
		wp_register_script(
			'wdc-toolkit-react-refresh-runtime',
			$react_fast_refresh_runtime,
			[],
			filemtime( $react_fast_refresh_runtime_path ),
		);

		// the hmr runtime is unique to a wdc-toolkit build
		$prefix          = $project . '-';
		$hmr_runtime_key = $prefix . 'wdc-toolkit-hmr-runtime';
		...
		if ( $script ) {
			...
			if( !in_array( 'wdc-toolkit-react-refresh-runtime', $script->deps ) ){
				$script->deps[] = 'wdc-toolkit-react-refresh-runtime';
			}

			if( !in_array( 'wdc-toolkit-react-fast-refresh-entry', $script->deps ) ){
				$script->deps[] = 'wdc-toolkit-react-fast-refresh-entry';
			}
...
add_action(
	...
	function() {
		...
		wp_enqueue_script( 'wdc-toolkit-react-refresh-runtime' );
...
add_action(
	...
	function() {
		$prefix          = basename( dirname( __DIR__ ) ) . '-';
		$hmr_runtime_key = $prefix . 'tewdcnup-toolkit-hmr-runtime';
