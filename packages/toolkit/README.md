...
With wdc-toolkit, engineers can quickly and easily bundle assets for both production and development without having
to worry about config files. wdc-toolkit is also easy to extend to project's specifics needs.

`wdc-toolkit` is inspired in tools like `react-scripts`, `kcd-scripts` and `wp-scripts`.

### Installation

To install wdc-toolkit simply run

```bash{showPrompt}
npm install --save-dev wdc-toolkit
```
...
In order to get `wdc-toolkit` up and running simply define the `source` and `main` properties in your `package.json` file.
You can also specify a `style` property to tell `wdc-toolkit` where to output your compiled css.
...
```json
{
	...
	"scripts": {
		"build": "wdc-toolkit build",
		"dev": "wdc-toolkit build --watch"
	}
}
```
...
wdc-toolkit can run in two different modes: package mode and project mode:
...
```json
{
	...
	"scripts": {
		...
		"build": "wdc-toolkit build",
		"format-js": "wdc-toolkit format-js",
		"lint-js": "wdc-toolkit lint-js",
		"lint-style": "wdc-toolkit lint-style",
		"test": "wdc-toolkit test-unit-jest"
	},
	"devDependencies": {
		"wdc-toolkit": "^1.0.0"
	},
...
Note the `wdc-toolkit` object in `package.json`. It exposes a few options to configure wdc-toolkit behavior.
...
When you run `wdc-toolkit build` with this configuration, wdc-toolkit will generate compiled assets for every entry point in the dist folder.
...
> Starting with wdc-toolkit@2.2.0 browser-sync has been deprecated. If you wish to continue using it you must install the following packages manually: npm install --save-dev browser-sync browser-sync-webpack-plugin

> It's strongly recommended to use the `--hot` option instead

wdc-toolkit has [BrowserSync](https://browsersync.io/) support and can be enabled by adding a devURL property.
...
In the example above, running `wdc-toolkit start` or `wdc-toolkit build --watch` will start wdc-toolkit in watch mode and start a browser sync session, proxying the *https://my-project.test* url.
...
_NOTE: Since wdc-toolkit@6 this `useBlockAssets` is on by default_
...
Since wdc-toolkit@6 this mode is on by default. To opt out of this mode you need to set the `useBlockAssets` option to your toolkit configuration to false:
...
wdc-toolkit provides native support for HMR and Fast Refresh with the `--hot` option. Fast Refresh works for general react development (including block development) and front-end CSS. Front-end vanilla JS will likely cause full-page refresh currently.

```
wdc-toolkit watch --hot
```
...
- If you're not using a `.test` domain for your WP instance, make sure to set `devURL` under `wdc-toolkit` namespace in `package.json`.
...
Make sure to reload the page after running wdc-toolkit as the `dist/fast-refresh.php` file is generated by wdc-toolkit
...
wdc-toolkit will lint/format any JavaScript or TypeScript file in your source code directory, you can customize this behavior in two ways:

- Specify the directory manually `wdc-toolkit lint-js src/`
...
```json
{
	"*.css": ["wdc-toolkit lint-style"],
	"*.[tj]s": ["wdc-toolkit lint-js"],
	"*.[tj]sx": ["wdc-toolkit lint-js"],
	// If you have php and phpcs
	"*.php": ["./vendor/bin/phpcs --extensions=php --warning-severity=8 -s"]
}
```

This will instruct lint-staged to run wdc-toolkit to link css, js and jsx files for the staged files in your commit.
...
wdc-toolkit leverages standard package.json fields when running in package mode. Below is a list of all fields wdc-toolkit supports and how it's used.

- **name**: It's used to generate a valid library name to expose the library via a global variable (for UMD builds).
  We recommend always specifying `libraryName` manually when creating UMD builds via `--name=your-library-name` or `wdc-toolkit.libraryName` config.
...
- **umd:main** or **unpkg**: wdc-toolkit generates a separate umd build by default. This field is used by wdc-toolkit to specify where the UMD bundle should go. The benefit of generating a separate UMD bundle is to avoid including boilerplate code.
...
It's important to understand how wdc-toolkit behaves when running in package mode. First and foremost, core-js polyfills **will not** be added automatically.
...
The second difference is that wdc-toolkit **wil not** include the dependencies (or peer dependencies) in the final bundle.
...
wdc-toolkit is very extensible and pretty much all config files can be overridden by simply creating a config file at the root of your project.
...
Refer to <Link to="/wdc-toolkit/linting">linting docs</Link> for more information.
...
By default, `wdc-toolkit` includes the following PostCSS plugins:
...
wdc-toolkit supports several CLI options that can be used to override settings.

### Bundle Analyzer

wdc-toolkit ships with `webpack-bundle-analyzer` out of the box, and you can enable it by simple passing the `--analyze` option.

`wdc-toolkit build --analyze`
...
```bash
wdc-toolkit build -i=src/index.js -o=dist/index.js
```
...
Then you can instruct wdc-toolkit to use your app.js file and spin up a dev server in a separate npm script.
...
<blockquote>This option was added in wdc-toolkit 1.0.8</blockquote>
...
wdc-toolkit is optimized for WordPress development and by default will include several WordPress specific settings. To disable this behavior
...
```bash
wdc-toolkit build --external=react,react-dom
```
...
```bash
wdc-toolkit build --include=package-name,other-package
```

## <a id="typescript"></a> TypeScript Support

wdc-toolkit provides support for TypeScript out of the box. Simply create files with `.ts` or `.tsx` (for react components) and wdc-toolkit will
...
There are two ways you can work with React in wdc-toolkit. When "WordPress" mode is turned on (the default behavior) wdc-toolkit will assume React is coming from WordPress and therefore will use `@wordpress/element`.

This is the default and expected behavior for writing custom gutenberg blocks for instance.

If you're writing React code on the front-end of your theme you can still use the bundled React that comes with WordPress. wdc-toolkit will automatically add `react`, `react-dom` and `wp-element` as dependencies of your front-end script that contains react code.
...
Once those packages are installed, wdc-toolkit will now look for any Linaria usage in JavaScript files and compile the CSS. The compiled CSS will have the name of the JS entry point (but with a .css extension).
...
wdc-toolkit will compile any Linaria code from the `index.js` entry point to `index.css`. Note that CSS imported by JS entry points always gets generated with the name of the entry point e.g: `index.js => index.css`. So make sure `editorStyle` points to the right CSS file which should be based on `editorScript`.
